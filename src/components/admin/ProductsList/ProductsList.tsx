...
const ProductsList: React.FC<IProductsListProps> = ({
  ...
  const handleChangeSelect = (option: Option) => {
    ...
    const isCategoryOptionSetToDefaultAndBrandOptionSelected =
      !option.id && option.field === "category" && selectedBrandOption.id;
    const isCategoryOptionSelectedAndBrandOptionIsDefault =
      option.field === "category" && !selectedBrandOption.id;
    const isBrandOptionSelectedAndSetCategoryOption =
      option.field === "category" && selectedBrandOption.id;
    ...
    const isBrandOptionSetToDefaultAndCategoryOptionSelected =
      !option.id && option.field === "brand" && selectedCategoryOption.id;
    const isCategoryOptionSelectedAndSetBrandOption =
      option.field === "brand" && selectedCategoryOption.id;
    const isBrandOptionSetAndCategoryOptionIsDefault =
      option.field === "brand" && !selectedCategoryOption.id;
    ...
    if (isCategoryOptionSelectedAndBrandOptionIsDefault) {
      const { filteredProducts, filteredBrands } = filterProductsByCategory(
        products,
        brands,
        option.id
      );
    ...
    if (isCategoryOptionSelectedAndSetBrandOption) {
      const { filteredCategories, filteredProductsByCategoryAndBrand } =
    ...
    if (isBrandOptionSelectedAndSetCategoryOption) {
      const { filteredBrands, filteredProductsByCategoryAndBrand } =
    ...
    if (isBrandOptionSetAndCategoryOptionIsDefault) {
      const { filteredProducts, filteredCategories } = filterProductsByBrand(
        products,
        categories,
        option.id
      );
                  ...
                  <tr key={id}>
                    ...
                    <td className={classes["product-name-cell"]}>
                      ...
                      <div className={classes["chip-wrapper"]}>
                        {discount && (
                          <Chip
                            text={"Скидка  -" + discount.percent + "%"}
                            mode={"highlighted"}
                          />
                        )}
                        {gender && (
                          <Chip
                            text={"Пол: " + gender.name.toLocaleLowerCase()}
                            mode={"info"}
                          />
                        )}
                      </div>
                    ...
                    <td className={classes["action-cell"]}>
                      <div className={classes.action}>
                        {LOCKED_PRODUCTS.includes(id) ? (
                          <Tooltip
                            icon={<LockIcon />}
                            message={NO_WAY_TO_CHANGE_THIS_PRODUCT}
                          />
                        ) : (
                          <>
